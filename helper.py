...
class PredictHelper:
    ...
    def get_past_vel_for_agent(self, instance_token: str, sample_token: str,
                               seconds: float, in_agent_frame: bool,
                               direction: str = 'prev',
                               just_xy: bool = True) -> Union[List[Record], np.ndarray]:
        ...
        coords = np.array([self.get_velocity_for_agent(r['instance_token'], r['sample_token']) for r in sequence])
        coords1 = np.array([self.get_acceleration_for_agent(r['instance_token'], r['sample_token']) for r in sequence])
        coords2 = np.array(
            [self.get_heading_change_rate_for_agent(r['instance_token'], r['sample_token']) for r in sequence])
        ...
        return np.concatenate((coords.reshape((-1, 1)), coords1.reshape((-1, 1)), coords2.reshape((-1, 1))), axis=1)
    ...
    def _get_past_or_future_for_sample(self, sample_token: str, seconds: float, in_agent_frame: bool,
                                       ...
                                       function: Callable[[str, str, float, bool, str, bool], np.ndarray]) -> Union[
        Dict[str, np.ndarray], Dict[str, List[Record]]]:
        """
...
def get_closest_lanes(x: float, y: float, radius: float = 50) -> List[str]:
    """
    ...
    lanes = NuScenesMap.get_records_in_radius(x, y, radius, ['lane', 'lane_connector'])
    lanes = lanes['lane'] + lanes['lane_connector']

    discrete_points = NuScenesMap.discretize_lanes(lanes, 0.5)

    min_id = []
    for lane_id, points in discrete_points.items():
        distance = np.linalg.norm(np.array(points)[:, :2] - [x, y], axis=1).min()
        if distance <= radius:
            min_id.append(lane_id)
